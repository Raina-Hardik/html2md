...
@click.option(
    '--rename',
    is_flag=True,
    help='Rename conflicting files with directory prefixes'
)
@click.option(
    '--skip',
    is_flag=True,
    help='Skip conflicting files instead of renaming'
)
@click.option(
    '--dry-run',
    is_flag=True,
...
@click.option(
    '--convert-as-inline',
    is_flag=True,
    help='Treat content as inline elements only'
)
@click.option(
    '--strip-newlines',
    is_flag=True,
...
@click.option(
    '--no-escape-asterisks',
    is_flag=True,
    help='Disable escaping of * characters'
)
@click.option(
    '--no-escape-underscores',
    is_flag=True,
    help='Disable escaping of _ characters'
)
@click.option(
    '--no-escape-misc',
    is_flag=True,
...
@click.option(
    '--code-language',
    default='',
    help='Default language for code blocks'
)
@click.option(
    '--parser',
...
@click.option(
    ...
    type=int,
    default=80,
    help='Text wrap width (default: 80)'
)
...
def main(
    ...
    if output_path and (flatten or len(html_files) > 1):
        ...
        if conflicts:
            if verbose:
                print_warning(f"Found {len(conflicts)} file(s) with naming conflicts", use_tqdm=False)
            ...
            if rename:
                ...
                file_mapping = ConflictResolver.filter_conflicts(conflicts, file_mapping)
                for conflict in conflicts:
                    print_warning(f"Skipping conflicted file: {conflict}", use_tqdm=False)
            else:
                print_error(f"Found {len(conflicts)} naming conflicts. Use --rename or --skip to resolve.", use_tqdm=False)
                if verbose:
                    for conflict in conflicts:
    ...
    if overwrite and not dry_run and success_count > 0:
        successful_inputs = [
            input_file for input_file, output_file in file_mapping.items()
            if processor.process_single_file(
                input_file, output_file, overwrite=False, dry_run=True
            )[0]
        ]
        
        _remove_original_files(
            successful_inputs, dry_run
        )
    ...
    if dry_run:
        ...
        if error_count == 0:
            ...
            print_warning(f"Converted {success_count}/{total_files} file(s), {error_count} error(s)")
